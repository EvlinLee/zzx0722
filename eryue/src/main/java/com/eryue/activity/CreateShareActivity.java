...
public class CreateShareActivity extends BaseActivity implements View.OnClickListener, GoodsSharePresenter.IGoodsShareDetailListener,
        ShareContentView.OnShareClickListener, ImageScrollView.OnCheckListener {
...
//        imageScrollView = (ImageScrollView) findViewById(R.id.image_scroll_view);
    ...
    private void initData() {
        ...
        sharePresenter.setShareDetailListener(this);

        String type = getIntent().getStringExtra("type");
        if (!TextUtils.isEmpty(type)) {
            if (type.equals("指定搜索")) {
                //指定搜索
                sharePresenter.superShareProductDetail(itemId, uid, searchFlag);
            } else if (type.equals("好券优选")) {
                sharePresenter.shareProductYxDetail(itemId, uid);
            } else if (type.equals("人气宝贝") || type.equals("爆款单") || type.equals("好货疯抢")) {
                //人气宝贝详情页面接口(人气宝贝、爆款单、好货疯抢)
                sharePresenter.shareTbActivityProductDetail(itemId, uid);
            } else if (type.equals("品牌爆款")) {
                //品牌爆款
                sharePresenter.shareProductBrandDetail(itemId, uid);
            } else if (type.equals("聚划算") || type.equals("淘抢购") || type.equals("视频购物")) {
                //聚划算详情页接口(聚划算、淘抢购、视频购物)
                sharePresenter.shareTbActivityProductDetail(itemId, uid);
            } else if (type.equals("收藏夹")) {
                //收藏夹
                sharePresenter.shareProductCollectionDetail(itemId, uid);
            }else {
                sharePresenter.shareProductDetail(itemId, uid);
            }
        } else {
            sharePresenter.shareProductDetail(itemId, uid);
        }
...
//        imageScrollView.setData(arrayList);
    ...
    public void onClick(View v) {
        if (v == tv_copy) {
            if (null != detailInfo && !TextUtils.isEmpty(detailInfo.content)) {
                StringUtils.copyToClipBoard(detailInfo.content,this);
            }
    ...
    public void onShareDetailBack(final InterfaceManager.ShareProductDetailInfo shareProductDetailInfo) {
        ...
        if (null == shareProductDetailInfo) {
            return;
        }
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                if (null!=shareProductDetailInfo.pics){

                    imgList = new ArrayList<>();
                    ImageScrollView.ImageModel imageModel;
                    for (int i=0;i<shareProductDetailInfo.pics.size();i++){

                        if (!TextUtils.isEmpty(shareProductDetailInfo.pics.get(i))){

                            imageModel = new ImageScrollView.ImageModel();
                    ...
                    imageScrollView.setData(imgList);

                }

                if (!TextUtils.isEmpty(shareProductDetailInfo.content)) {

                    String content = shareProductDetailInfo.content;
                    //自定义小尾巴
    ...
    public void onShareClick(final int tag) {
        ...
        if (null != wxShare) {
                            ...
                            public void handleMessage(Message msg) {
                                if (null != shareImgFiles && !shareImgFiles.isEmpty()) {
                                    if (tag == ShareContentView.FLAG_WEIXIN) {
                                        ...
                                        wxShare.shareMultiplePictureToFriend(CreateShareActivity.this, shareImgFiles);
                                        //复制内容到剪贴板
                                        StringUtils.copyToClipBoard(detailInfo.content,CreateShareActivity.this);
                                    } else if (tag == ShareContentView.FLAG_WEIXIN_TIMELINE) {
                                        wxShare.shareMultiplePictureToTimeLine(CreateShareActivity.this, detailInfo.content, shareImgFiles);
                                    }
                                }
            ...
            ImageUtils.getInstance(CreateShareActivity.this).download(urlList);
